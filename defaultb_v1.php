<?php
//! Clase   session_start(); se omitio 

use Illuminate\Http\Request; //! Clase  
...
//Configuracion de la conexion a base de datos
    ...
    #endregion
	#region require('validapost.php'); //garm20220827 //!>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>



	if ($request()->has('s_transm')) {
		$wsecuencia = substr($request()->input('s_transm'), 0, 20);
	} else {
		$wsecuencia = "0";
	}
	// Referencia "Referencia única e irrepetible por proceso de pago" char de 20 "EXPEDIENTE"

	if ($request()->has('c_referencia')) {
		$wreferencua  = $request()->input("c_referencia");  
		$Expe		 = $request()->input("c_referencia"); 	
	} else {
		$wsecuencia = "0";
	}
	...
	if($request()->has('t_servicio') ) {
	
		$wservicio  = $request()->input("t_servicio");	
	...
	if($request()->has('t_importe') ) {
	
		$wimporte  =  $request()->input("t_importe");	
	...
	// Tipo de Pago
	if($request()->has('t_pago') ) {
	
		$t_pago  =  $request()->input("t_pago");	
	...
	// Nivel 1 De Detalle
	if($request()->has('val_1') ) {
	
		$wval_1  =  $request()->input("val_1");	
	...
	if($request()->has('val_5') ) {
	
		$val_5  =  $request()->input("val_5");	
	...
	if($request()->has('val_6') ) {
	
		$val_6  =  $request()->input("val_6");	
	...
	// Número de Tarjeta
	if($request()->has('val_9') ) {
	
		$val_9  =  $request()->input("val_9");
		$numtc       = ('XXXXXXXXXX'. substr($request()->input("val_9"),0,4));  // OK	
	...
	// Fecha de Pago
	if($request()->has('val_10') ) {
	
		$val_10  =  $request()->input("val_10");
	...
	// E-Mail
	if($request()->has('val_11') ) {
	
		$val_11  =  $request()->input("val_11");	
	...
	// Teléfono
	if($request()->has('val_12') ) {
	
		$val_12  =  $request()->input("val_12");	
	...
	// HMAC SHA-1
	if($request()->has('val_13') ) {
	
		$val_13  =  $request()->input("val_13");	
	...
	if($request()->has('n_autoriz') ) {
	
		$noperacion  =  $request()->input("n_autoriz");
		...
		//	$val_13		 = @$request()->input("val_13");  							// HMAC SHA-1
		//}
			/////////////////////////////// validapost.php//////////////////////////////////////
		   /////////////////////////////// HMAC //////////////////////////////////////
		   //////////////////////////////2023-01-24//////////////////////////////////////
	...
	$hs_transm = substr($request()->input("s_transm"),0,20);
	$hc_referencia = $request()->input("c_referencia");   
	$h_totalAdeudo = $request()->input("t_importe");
	$hn_autoriz = $request()->input("n_autoriz");      
	...
	$hmacadquira = $request()->input("mp_signature");
...
//	$wsecuencia  = substr($request()->input("s_transm"],0,20);				// Secuencia de Transmisi�n "Secuencia � folio que identifica transacci�n para MUNICIPIO DE GUADALUPE. �nico e irrepetible" numeric de 20
//	$wreferencua = $request()->input("c_referencia");  					// Referencia "Referencia �nica e irrepetible por proceso de pago" char de 20 "EXPEDIENTE"
//	$wval_1		 = $request()->input("val_1");  							// Nivel 1 De Detalle
//	$wservicio   = $request()->input("t_servicio");	   					// Tipo de Servicio "POR DEFINIR" num de 3 --002 predial--
//	$wimporte    = $request()->input("t_importe");	               			// Importe Total "9 enteros 2 decimales con punto" num 9,2
//	$val_3		 = $request()->input("val_3");  							// Moneda
//	$t_pago		 = $request()->input("t_pago");  							// Tipo de Pago
//	$noperacion  = $request()->input("n_autoriz");							// Numero de operacion bancaria	 // GARM 20220811 no debe ser null vacio o en cero 		
//	$val_9		 = $request()->input("val_9");  							// N�mero de Tarjeta
//	$val_10		 = $request()->input("val_10");  							// Fecha de Pago
//	$val_5		 = $request()->input("val_5");  							// Financiamiento// garm 20220811  1 no aplica  2 aplica meses en   solo en BBVA TARJETAHABIENTES FINANCIAMIENTO 
//	$val_6		 = $request()->input("val_6");  							// Periodo de Financiamiento
//	$val_11		 = $request()->input("val_11");  							// E-Mail
//	$val_12		 = $request()->input("val_12");  							// Tel�fono
	
//	$numtc       = 'XXXXXXXXXX'.substr($request()->input("val_9"],0,4);	// Numero de tarjeta 	
//	$Expe		 = $request()->input("c_referencia"); 
...
switch($wvarpaso)
{
	case "CONEXP":
		...
		//EXTRAE NUMERO CONSECUTIVO DE RECIBO
		 ...
		 $result = DB::select(DB::raw($sqlpru)); //! Clase  
