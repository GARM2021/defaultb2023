...
class DefaultbController extends Controller
{
	function TotalesPredialLinea()
	{
		...
		$result = DB::connection('sqlsrv')->select("SELECT     COUNT(DISTINCT exp) AS cuantos, ctafolio FROM         preddpagos WHERE  (estatus <=0) and (fpago > '20230100') and  (NOT (caja IN ('0805', '0806', '0811',  '0705')))  GROUP BY ctafolio ");
		...
		$result = DB::connection('sqlsrv')->select("SELECT     COUNT(DISTINCT exp) AS cuantos, ctafolio FROM         preddpagos WHERE   (estatus <=0) and  (fpago > '20230100')  GROUP BY ctafolio ");
		...
		return view('totaLineaPred', ["resultados" => $datos]);
	}
}
